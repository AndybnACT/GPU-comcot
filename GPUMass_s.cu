...
__global__ void mass_kernel_pitched(const float2* __restrict__, const float2* __restrict__,
...
extern "C" void mass_launch_(const float* Z_f, float* Z_f_complete, const float *H_f){
    ...
    st = clock();
    mass_kernel_pitched <<< DimGridMass, DimBlockMass >>> (ZHdat_pitchedMEM_hst, MNdat_pitchedMEM_hst, R_MASS_hst);
...
__global__ void mass_kernel_pitched(const float2* __restrict__ ZH, const float2* __restrict__ MN,
    ...
    n_prev = F2_PITCH_ACCESS(MN, LayerPitch_dev, row, col)->y;//MN[ID2E(row,col,1)];
    r6_prev = R_MASS[col*4+1];
    ...
    for (uint32_t i = col+1; i < col_end; i++) {
        ...
        r6 = __shfl_sync(0xFFFFFFFF,r6,0);
        r11 = __shfl_sync(0xFFFFFFFF,r11,0);
        m = F2_PITCH_ACCESS(MN, LayerPitch_dev, row, i)->x;//MN[ID(row,i)];
        n = F2_PITCH_ACCESS(MN, LayerPitch_dev, row, i)->y;//MN[ID2E(row,i,1)];
        h = F2_PITCH_ACCESS(ZH, LayerPitch_dev, row, i)->y;//
        z = F2_PITCH_ACCESS(ZH, LayerPitch_dev, row, i)->x;//Z[ID(row,i)];
        m_suf = __shfl_up_sync(0xFFFFFFFF,m,1);
        if (threadIdx.x%32 != 0 && row < size_dev[0]-1) {
            ztmp = z - r1*(m-m_suf) - r11*(n*r6-n_prev*r6_prev);
            if (ztmp + h <= EPS) ztmp = -h;
            ...
            F2_PITCH_ACCESS(ZHout_pitchedMEM_dev, LayerPitch_dev, row, i)->x = ztmp;// Z_out_dev[ID(row,i)] = ztmp;
            // if (row == 2807 && i == 1477) {
