...
extern "C" void openbd_launch_(float *Z_f_complete) {
    ...
    openbd_kernel_tex_<<< DimGridOpenBD_LR, DimBlockOpenBD, 0, EXECstream[0] >>>(LEFT);// MN has been changed (:,:,1) <--> (:,:,2)
    openbd_kernel_tex_<<< DimGridOpenBD_LR, DimBlockOpenBD, 0, EXECstream[1] >>>(RIGHT);// so use M(:,:,1) directly
    openbd_kernel_tex_<<< DimGridOpenBD_TB, DimBlockOpenBD, 0, EXECstream[2] >>>(TOP);
    openbd_kernel_tex_<<< DimGridOpenBD_TB, DimBlockOpenBD, 0, EXECstream[3] >>>(BOTTOM);
    cudaDeviceSynchronize();
    err = cudaGetLastError();
    cudaERROR(err);
...
__global__ void openbd_kernel_tex_(bdside BOUNDARY) {
    #define UB 99.0
    float ztmp=0.0, h, m, n, cc;
    uint32_t row=0, col=0;
    switch (BOUNDARY) {
        case RIGHT:
            ...
            row = blockIdx.x*31*(blockDim.x>>5) + 31*(threadIdx.x>>5) + threadIdx.x%32;
            if (row < size_dev[0]-1) {
                h = F2_PITCH_ACCESS(ZHout_pitchedMEM_dev, MNpitch_dev, row, size_dev[1]-1)->y;//h =  H[row+col];
                m = tex2D<float2>(MNtext, row, size_dev[1]-1).x;//MN[row+col];// must not load in the following if block, or 1st lane would get 0
                float m_suf = __shfl_up_sync(0xFFFFFFFF,m,1);// must not shuffle in the following if block, or 1st lane would get 0
                n = tex2D<float2>(MNtext, row, size_dev[1]-2).y;//MN[row+col-size_dev[0]+size_dev[2]];
                cc = 1/sqrtf(GRAV*h);
                if (threadIdx.x % 32 != 0) {
                    ...
                    // Z_out_dev[row+col] = ztmp;
                    ANSCHK(Z_out_dev[row+col], ztmp);
                    F2_PITCH_ACCESS(ZHout_pitchedMEM_dev, MNpitch_dev, row, size_dev[1]-1)->x = ztmp;
                }
                else if (row == 0) { // --|
                    if (h > GX) {
                        ztmp = sqrtf(m*m + n*n)*cc;
                    ...
                    // Z_out_dev[col] = ztmp;
                    ANSCHK(Z_out_dev[col], ztmp);
                    F2_PITCH_ACCESS(ZHout_pitchedMEM_dev, MNpitch_dev, 0, size_dev[1]-1)->x = ztmp;
                }
            }
            break;
        case LEFT:
            row = blockIdx.x*31*(blockDim.x>>5) + 31*(threadIdx.x>>5) + threadIdx.x%32;
            if (row < size_dev[0]-1) {
                h = F2_PITCH_ACCESS(ZHout_pitchedMEM_dev, MNpitch_dev, row, 0)->y; //H[row];
                m = tex2D<float2>(MNtext, row, 0).x;//MN[row];
                float m_suf = __shfl_up_sync(0xFFFFFFFF,m,1);
                n = tex2D<float2>(MNtext, row, 0).y;//MN[row+size_dev[2]];
                cc = 1/sqrtf(GRAV*h);
                if (threadIdx.x % 32 != 0) {
                    ...
                    // Z_out_dev[row] = ztmp;
                    ANSCHK(Z_out_dev[row], ztmp);
                    F2_PITCH_ACCESS(ZHout_pitchedMEM_dev, MNpitch_dev, row , 0)->x = ztmp;
                }
                else if (row == 0) {//  |--
                    if (h > GX) {
                        ztmp = sqrtf(m*m + n*n)*cc;
                    ...
                    // Z_out_dev[col] = ztmp;
                    ANSCHK(Z_out_dev[col], ztmp);
                    F2_PITCH_ACCESS(ZHout_pitchedMEM_dev, MNpitch_dev, 0, 0)->x = ztmp;
                }
            }
            break;
        case TOP: // should use texture
            ...
            if (col < size_dev[1]-1 && col > 0) {
                col *= size_dev[0];
                h = F2_PITCH_ACCESS(ZHout_pitchedMEM_dev, MNpitch_dev, 0, col/size_dev[0])->y; //H[col];
                n = tex2D<float2>(MNtext, 0, col/size_dev[0]).y;//MN[col+size_dev[2]];
                float n_suf = __shfl_up_sync(0xFFFFFFFF,n,1);
                if (threadIdx.x%32 != 0) {
                    if (h > GX) {
                    ...
                    // Z_out_dev[col] = ztmp;
                    ANSCHK(Z_out_dev[col], ztmp);
                    F2_PITCH_ACCESS(ZHout_pitchedMEM_dev, MNpitch_dev, 0, col/size_dev[0])->x = ztmp;
                }
            }
            break;
        case BOTTOM:
            ...
            if (col < size_dev[1]-1 && col > 0) { //bottom body
                col *= size_dev[0];
                h = F2_PITCH_ACCESS(ZHout_pitchedMEM_dev, MNpitch_dev, row, col/size_dev[0])->y;// H[col+row];
                n = tex2D<float2>(MNtext, row, col/size_dev[0]).y;//MN[col+row+size_dev[2]];
                float n_suf = __shfl_up_sync(0xFFFFFFFF,n,1);
                if (threadIdx.x%32 != 0) {
                    if (h > GX) {
                    ...
                    // Z_out_dev[col+row] = ztmp;
                    ANSCHK(Z_out_dev[col+row], ztmp);
                    F2_PITCH_ACCESS(ZHout_pitchedMEM_dev, MNpitch_dev, row, col/size_dev[0])->x = ztmp;
                }
            }else if (col == 0 || col == size_dev[1]-1){ // bottom boundary
                // uint32_t id = row+col*size_dev[0];
                h = F2_PITCH_ACCESS(ZHout_pitchedMEM_dev, MNpitch_dev, row, col)->y; //H[id];
                if (h > GX) {
                    cc = 1/sqrtf(GRAV*h);
                    m = tex2D<float2>(MNtext, row-1, col).x;//MN[id-1];
                    if (col == 0) {  // |__
                ...
                F2_PITCH_ACCESS(ZHout_pitchedMEM_dev, MNpitch_dev, row, col)->x = ztmp;
            }
    }
}
