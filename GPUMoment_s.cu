...
__global__ void momts_kernel_tex(const float* __restrict__ , const float* __restrict__ );
...
extern "C" void momt_launch_(float *M_f, float *N_f, float *Z_f /*FUTURE: delete*/) {
    ...
    cudaCHK( cudaBindTexture2D(NULL, ZHtext, ZHout_pitchedMEM_hst, descflt2, size_hst[0], size_hst[1], LayerPitch) );
    st = clock();
    momts_kernel_tex <<< DimGridMomt, DimBlockMomt >>> (R24_hst, R35_hst);
...
__global__ void momts_kernel(const float* __restrict__ Z, const float* __restrict__ MN,
    ...
    //printf("%d %d %d\n",row, col, id);
        ...
        MN_dat[threadIdx.x][threadIdx.y][threadIdx.z] = MN[idmn]; //(threadIdx.z==0) ? F2_PITCH_ACCESS(MN, LayerPitch_dev, row,col)->x:F2_PITCH_ACCESS(MN, LayerPitch_dev, row,col)->y;
        __syncthreads();
...
__global__ void momts_kernel_tex(const float* __restrict__ R24, const float* __restrict__ R35) {
    __shared__ float   Z_dat[BLOCKX][BLOCKY+1]; // BLOCKY+1 for preventing bank conflicts
    // __shared__ float  MN_dat[BLOCKX][BLOCKY+1][2];
    __shared__ float   H_dat[BLOCKX][BLOCKY+1];
                    ...
                    if (row < size_dev[0] - 1){ // IS:IE
                        ...
                        if (H_dat[threadIdx.x][threadIdx.y] > GX && H_dat[ip1][threadIdx.y] > GX) { //NOTE redundant checks, since H is constant
                            ...
                            x   = tex2D<float2>(MNtext, row, col).x - r1*(Z_dat[ip1][threadIdx.y] - Z_dat[threadIdx.x][threadIdx.y]) +
                                            r2*tot;
                            // ANSCHK((F2_PITCH_ACCESS(MNout_pitchedMEM_dev, LayerPitch_dev, row, col)->x), x);
                            // if (fabs(F2_PITCH_ACCESS(MNout_pitchedMEM_dev, LayerPitch_dev, row, col)->x - x) > 1e-4 && row == 2806 && col == 1477) {
                            //     printf("XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX\n");
                            //     // printf("%s Kernel/Kernel_[%d,%d] inconsistent: Kernel%e\tKernel_:%e\n",__FILE__,row,col,F2_PITCH_ACCESS(MNout_pitchedMEM_dev, LayerPitch_dev, row, col)->x, x);
                            // }
                            if ( x > EPS || -x > EPS) F2_PITCH_ACCESS(MNout_pitchedMEM_dev, LayerPitch_dev, row, col)->x = x;//MN_out_dev[id] = tot;
                            else  F2_PITCH_ACCESS(MNout_pitchedMEM_dev, LayerPitch_dev, row, col)->x = 0.0f;
                    ...
                    if (col < size_dev[1] - 1) { //JS:JE
                        ...
                        if (H_dat[threadIdx.x][threadIdx.y] > GX && H_dat[threadIdx.x][jp1] > GX) {  // preconditions
                            ...
                            x    = tex2D<float2>(MNtext, row, col).y - r1*(Z_dat[threadIdx.x][jp1] - Z_dat[threadIdx.x][threadIdx.y]) -
                                              r2*tot;
                            // ANSCHK((F2_PITCH_ACCESS(MNout_pitchedMEM_dev, LayerPitch_dev, row, col)->y), x);
                            if ( x > EPS || -x > EPS)  F2_PITCH_ACCESS(MNout_pitchedMEM_dev, LayerPitch_dev, row, col)->y = x;//MN_out_dev[idmn] = tot;
                            else F2_PITCH_ACCESS(MNout_pitchedMEM_dev, LayerPitch_dev, row, col)->y = 0.0f;
